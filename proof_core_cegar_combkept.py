...
def _expand_kept_regs_with_comb_predecessors(dep_g, kept_nodes, state_vars, max_steps=None):
    regs = set()
    visited = set()
    queue = [(node, 0) for node in kept_nodes]
    while queue:
...
def run_cegar(
    ...
    if not depths:
        return "no_state", None, None, None
    ...
    _run_yosys_smt2(
        ...
        smt2_path,
        include_dirs=include_dirs,
        flatten=_needs_flatten(assertion_expr),
    )
    ...
    # Disable pure-combinational shortcut in this experimental file so that
    # CEGAR + expanded kept can attempt to produce a proof core.

    for depth in depths:
        a1, kept, _ = compute_a1_for_targets(dep_g, targets, state_vars, depth=depth)
        kept_regs = {reg for reg in state_vars if reg in kept}
        comb_regs = _expand_kept_regs_with_comb_predecessors(
            dep_g, kept, state_vars, max_steps=None
        )
        if comb_regs:
            kept_regs |= comb_regs
