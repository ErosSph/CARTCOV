...
import re # To search for variable names2
...
def _comment_sv_line(line):
    return "// __sv_assert_stripped__\n" if line.endswith("\n") else "// __sv_assert_stripped__"


def _normalize_unpacked_array_decl(line):
    if ";" not in line:
        return line

    def repl(match):
        size = match.group(2).strip()
        return "%s[0:%s-1]" % (match.group(1), size)

    return re.sub(r"(]\s*[A-Za-z_][A-Za-z0-9_$]*\s*)\[(\s*[^:\]]+)\]", repl, line)


def _preprocess_sv_text(text):
    lines = text.splitlines(True)
    out_lines = []
    in_assert = False
    for line in lines:
        if in_assert:
            out_lines.append(_comment_sv_line(line))
            if ";" in line:
                in_assert = False
            continue
        if re.search(r"\bassert\s+property\b", line):
            in_assert = True
            out_lines.append(_comment_sv_line(line))
            if ";" in line:
                in_assert = False
            continue
        if re.search(r"`assert\b", line):
            out_lines.append(re.sub(r"`assert\b.*", ";", line))
            continue

        processed = line
        processed = re.sub(r"\(\*.*?\*\)", " ", processed)
        processed = re.sub(r"\balways_ff\b", "always", processed)
        processed = re.sub(r"\balways_comb\b", "always @*", processed)
        processed = re.sub(r"\bunique\s+case\b", "case", processed)
        processed = re.sub(r"\bpriority\s+case\b", "case", processed)
        processed = re.sub(r"\bparameter\s+int\b", "parameter", processed)
        processed = re.sub(r"\blocalparam\s+int\b", "localparam", processed)
        processed = re.sub(r"\blogic\b", "wire", processed)
        processed = re.sub(r"\bbit\b", "wire", processed)
        processed = re.sub(r"'\s*0\b", "0", processed)
        processed = re.sub(r"'\s*1\b", "1", processed)
        processed = re.sub(
            r"(\d+)'([bBdDhHoO])\s+([0-9a-fA-F_xXzZ])",
            r"\1'\2\3",
            processed,
        )
        processed = re.sub(r"\.(\w+)\s*(?=,|\))", r".\1(\1)", processed)
        match = re.match(r"(\s*)repeat\s*\(([^)]+)\)\s*@\(([^)]+)\)\s*;\s*$", processed)
        if match:
            indent, count, event = match.groups()
            if re.fullmatch(r"\d+", count.strip()):
                repeat_count = int(count.strip())
                processed = "%s%s\n" % (
                    indent,
                    " ".join(["@(%s);" % event] * repeat_count),
                )
            else:
                processed = "%s// __sv_repeat_stripped__\n" % indent
        processed = _normalize_unpacked_array_decl(processed)
        out_lines.append(processed)
    return "".join(out_lines)


def _preprocess_sv_files(vfilelist):
    temp_dir = tempfile.mkdtemp(prefix="sv_pre_")
    new_files = []
    for idx, vfile in enumerate(vfilelist):
        with open(vfile, "r", encoding="utf-8", errors="ignore") as handle:
            text = handle.read()
        processed = _preprocess_sv_text(text)
        out_path = os.path.join(temp_dir, "%d_%s" % (idx, os.path.basename(vfile)))
...
def _parse_verilog_files(vfilelist, include, define):
    parsed = parse(
        vfilelist,
    ...
    if len(parsed) == 3:
        ast, directives, preprocessed_code = parsed
    else:
...
def parse_verilog(vfilelist, include, define):
    try:
        return _parse_verilog_files(vfilelist, include, define)
    except ParseError:
        return _parse_verilog_files(_preprocess_sv_files(vfilelist), include, define)
...
def _parse_int_const(value):
    text = str(value).replace(" ", "")
    match = re.match(r"(?i)^(\d+)?'s?([bdho])([0-9a-f_xz]+)$", text)
    if match:
        ...
        return int(digits, base_map[base.lower()])
    if re.fullmatch(r"\d+", text):
        return int(text)
    return None


def _eval_param_expr(ast, resolver):
    if ast is None:
        return None
    ...
    if kind == "Rvalue":
        ...
        if not children:
            return None
        return _eval_param_expr(children[0], resolver)
    if kind == "Lvalue":
        children = ast.children()
        if not children:
            return None
        return _eval_param_expr(children[0], resolver)
    if kind in ("IntConst", "Constant"):
        return _parse_int_const(ast.value)
    if kind == "Identifier":
        return resolver(ast.name)
    if kind in ("Uplus", "Uminus", "Unot", "Ulnot"):
        val = _eval_param_expr(ast.right, resolver)
        if val is None:
            return None
        if kind == "Uplus":
            return val
    ...
    if kind in (
        ...
        left = _eval_param_expr(ast.left, resolver)
        right = _eval_param_expr(ast.right, resolver)
    ...
    if kind == "Cond" or (
        hasattr(ast, "cond")
        and hasattr(ast, "true_value")
        and hasattr(ast, "false_value")
    ):
        cond = _eval_param_expr(ast.cond, resolver)
        if cond is None:
            return None
        branch = ast.true_value if cond else ast.false_value
        return _eval_param_expr(branch, resolver)
    return None
...
def get_params(ast, top_module):
    ...
    locals_ = {}
    module_stat = False
    get_params_(ast, top_module, params, locals_, module_stat)
    ...
    def resolve(name):
        ...
        if name in resolving:
            print_fail("Parameter dependency cycle detected: %s" % name)
            return None
        if name in params:
            expr = params[name]
            ...
            return None
        resolving.add(name)
        val = _eval_param_expr(expr, resolve)
        resolving.remove(name)
        if val is not None:
            values[name] = val
        return val

    for name in list(params.keys()) + list(locals_.keys()):
        resolve(name)

    return values


def get_params_(ast, top_module, params, locals_, module_stat):
    if (ast.__class__.__name__ == 'ModuleDef'):
        if ast.name == top_module:
            module_stat = True
        else:
            module_stat = False
    elif module_stat and ast.__class__.__name__ in ('Parameter', 'Localparam'):
        param_name = ast.name
        param_value = ast.value
        if ast.__class__.__name__ == 'Parameter':
            params[param_name] = param_value
        else:
            locals_[param_name] = param_value

    for c in ast.children():
        get_params_(c, top_module, params, locals_, module_stat)

    return

def get_ports(ast, top_module, Params=None):
...
def flatten_port_width(ast, Params):
    ...
    if ast.__class__.__name__ == 'Plus':
        ...
        try:
            ...
            print_fail('Port width cannot be determined (missing parameter: %s). Aborting' % ast.name)
