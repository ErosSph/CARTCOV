...
# Set to True for verbose AST/debug printing.
VERBOSE = False
...
def _is_slang_node(node):
    return pyslang is not None and isinstance(node, pyslang.SyntaxNode)


def _slang_extract_identifiers(node):
    names = []

    def scoped_name(n):
        if not _is_slang_node(n):
            return None
        ...
        if n.kind == SyntaxKind.IdentifierSelectName:
            tok = n.identifier
            return tok.value if tok is not None else None
        if n.kind == SyntaxKind.ScopedName:
            left = scoped_name(n.left)
            right = scoped_name(n.right)
            if left and right:
                return left + "." + right
        return None
    ...
    def walk(n):
        ...
        if n.kind == SyntaxKind.IdentifierSelectName:
            ...
            return
        if n.kind == SyntaxKind.ScopedName:
            scoped = scoped_name(n)
            if scoped:
                names.append(scoped)
            return
        for child in n:
            if _is_slang_node(child):
                walk(child)
...
def _is_nonblocking_expr(expr):
    if _is_slang_node(expr):
        return expr.kind == SyntaxKind.NonblockingAssignmentExpression
    return getattr(expr, "__class__", type("x", (), {})).__name__ == "NonblockingSubstitution"
...
def construct_CDFG(ast, CDFGS, MODINSTS, clks, indent):#通过DFS构建CDFG
    ...
    if (ast.__class__.__name__ == 'Always'):
        ...
        get_rhs_cond_nodes(ast.sens_list, sens_list)#sens_list是敏感列表
    ...
    elif (ast.__class__.__name__ == 'Assign'):#这个是数据流相关的，这个是为了得到左边的数据流向，数据往哪里流动了才是重要的
        ASGraph = nx.DiGraph()
        typ = ast.__class__.__name__
        ...
        instance_name = ast.name#对于实例化我需要什么呢？
        ...
        for port in portlist:
            ...
            pname = port.portname
            aname = []
            get_rhs_cond_nodes(port.argname, aname) #对于端口实例化，我寻找的是右边传进来的是什么
            #print(pname + ' ' + ', '.join(aname))
            portmap[pname] = aname
        
        MODINSTS[instance_name] = [module_name, portmap]

    for c in ast.children():#ast的下一个节点的内容
        #c.show()
...
def traverse_CDFG(CDFG):
    ...
    nodes = CDFG.nodes()#到底是那一步让这里的node有了这么多节点？分而治之，通过把一块一块的内容拿出来，然后再进行处理，产生节点
...
def CDFG(ast, clks, Params, Ports):
    ...
    for i in range(len(CDFGS)):#看起来CDFG的构建是从一个点出发去进行构建，这个点的选取如果没有理解错就是
        if VERBOSE:
            print(CDFGS[i].nodes())
    for CDFG in CDFGS:
        cdfg_type = CDFG.nodes[list(CDFG.nodes())[0]]['typ']
        if cdfg_type == 'Always':
            cdfg_ast =  CDFG.nodes[list(CDFG.nodes())[0]]['ast']#这个模块的ast都被放了进来
    ...
    for CDFG in CDFGS:
        ...
        if cdfg_type == 'Always':
            PathSets.append(traverse_CDFG(CDFG))#CDFG的代码有点看不懂
...
def get_var_def_chain(CDFGS, PathSets, Params):
    ...
    for idx1 in range(len(CDFGS)):
        ...
        path_key_set = set(itools.chain(*PathSet))#把path展平之后再去掉重复的
        CDFG_key_set = set(CDFG.nodes())

        if list(set.intersection(path_key_set, CDFG_key_set)):#来判断是否有公共元素，如果有就让path_index+1
            path_index = path_index + 1
            ...
            for Path in PathSet:
                ...
                for idx2 in range(1, len(Path)):
                    ...
                    if statements:
                        ...
                        c_lines = clines[:]
                        for statement in statements:
                            def_var = get_lvalue(statement.left)#在 Verilog/SystemVerilog 语法里，always 块（以及 initial 块）里面写的每一条执行单元都叫 statement（语句）。
                            if VERBOSE:
                                statement.show()
                            def_vars.append(def_var)
                            d_dep = []
                            if not get_rhs_constants(statement.right):#看数据依赖的右边是不是常量，如果不是常量才进行分析
                                if VERBOSE:
                                    statement.right.show()
                                get_rhs_cond_nodes(statement.right, d_dep)
                                for dep in d_dep:
                                    if dep in list(Params.keys()):
                                        d_dep.remove(dep)
    
                            if def_var not in list(var_def_chain.keys()):#所以这些以来都是从ast中获得的？
                                var_def_chain[def_var] = {'CDeps': [c_dep],#控制依赖
                                                          'DDeps': [d_dep],#数据依赖
                                                          'Clocked': clocked,#是否是时钟逻辑
                                                          'Expressions': [statement],#原始定义语句
                                                          'CLines': [c_lines],#控制依赖的源码行号
                                                          'DLines': [str(statement.lineno) + ':D']#数据依赖的源码行号
                                                          }
                            else:#CDeps还是不太清楚，那这样吧，需要用到的时候再细看
                                var_def_chain[def_var]['CDeps'].append(c_dep)#有的有依赖有的没依赖，我觉得应该是结合ast来看有的ast部分有依赖，那没依赖的ast部分是不是可以去掉了
                        ...
                        # as accumulated from the different paths of an Always CDFG. Same variable
                        # can be defined in different paths. 需要集合操作来合并，保证最后记录的是唯一的
                        def_vars = list(set(def_vars + CDFG.nodes[leaf_node]['def_var']))
                        CDFG.nodes[leaf_node]['def_var'] = def_vars#这里是为什么？不是很理解
                    ...
                    cond.append(CDFG.get_edge_data(*edge)['cond'])#这里应该没有条件语句
...
def dependencies(var, ELABORATE_INFO, scope_module_map, top_module, dep_g, k, kmax):
    ...
    for dep in deps_from_dep_g:
        ...
        if found[0] != None:
            ...
            exp_typ = expression.__class__.__name__ if not _is_slang_node(expression) else None
            #print exp_typ
            if _is_nonblocking_expr(expression):
                nxt_k = k + 1
                if nxt_k < kmax:
...
def expr_length(Comm_Expr):
    ...
    if type(Comm_Expr) is list:
        ...
        if _is_slang_node(Comm_Expr):
            Length = len(_slang_extract_identifiers(Comm_Expr))
        else:
            Expr_Var = []
...
def expressions(var, top_module, scope_module_map, ELABORATE_INFO):
    # FIXME: line no for control statements #使用链和定义链分别代表了expression 和 敏感度
    scope = var[1]
    module = top_module if not scope else scope_module_map[scope]
    var_def_chain = ELABORATE_INFO[module]['var_def_chain']
    v = var[2]
...
def Linking(ELABORATE_INFO, top_module):
    ...
    while InstanceQueue:
        ...
        for inst in list(curr_modinsts.keys()):
            ...
            for pname in list(portmap.keys()):#为什么这里跑了一下就退出了？不是很理解
                ...
                # NOTE: For CDFG
                node3 = []
                if pname in list(p_output_to_be_connected.keys()):#这里真的没啥问题吗？如果有问题呢？
    ...
    #       part of any CDFGs but some intermediate wire variable used to connect different CDFGs,不能对容器直接进行修改，需要拍快照来进行修改
    for enode in excess_nodes_added_in_fused_CDFG:
        predecessors = list(complete_fused_CDFG.predecessors(enode))
        successors = list(complete_fused_CDFG.successors(enode))
